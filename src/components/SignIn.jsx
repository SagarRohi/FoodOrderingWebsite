...
import { setUser ,setsignInShow, toggle, setsignUpShow} from '../reducer';
...
const SignIn = () => {
  ...
  return (
    ...
    flex-col z-[101]'>
       ...
       p-4'>
           <motion.div onClick={()=>dispatch(setsignInShow(false))}  whileTap={{scale:0.75}}>
           <MdOutlineKeyboardBackspace className='text-textColor
            text-3xl'/>
           </motion.div>
            <p className='text-textColor text-lg
            font-semibold'>Sign In</p>
        ...
        <div className="relative w-[90%]">
         ...
         <motion.button onClick={handleSignIn} whileTap={{scale:0.90}} className='hover:bg-orange-500 w-[90%] px-8 py-2 text-lg 
         bg-orange-600 text-white rounded-full flex justify-center '>{loading?<ThreeDots color="#00000" height={20} width={50}  />:"Sign In"}
         </motion.button>
         ...
         <div className='flex text-sm font-thin items-center gap-2 text-white'>
          ...
          <div className='mx-2 h-full w-0 border-l-2 border-black'></div>
          <motion.div whileTap={{scale:0.90}} onClick={()=>dispatch(toggle())}
          className='text-white text-sm font-extralight cursor-pointer hover:text-orange-500'>Create new Account</motion.div>
         </div>
        </div>
